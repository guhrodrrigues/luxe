...
function TextGlitch({ text }: { text: string }) {
  return (
    <div className="relative overflow-hidden">
      <span className="invisible font-mono flex items-center gap-1">
        {text} <ChevronRightIcon size={14} />
      </span>
      <span className="flex items-center gap-1 font-semibold font-mono absolute top-0 left-0 group-hover:-translate-y-full transition-transform ease-in-out duration-500 hover:duration-300">
        {text}
        <ChevronRightIcon size={14} />
      </span>
      <span className="flex items-center gap-1 font-semibold font-mono absolute top-0 left-0 translate-y-full group-hover:translate-y-0 transition-transform ease-in-out duration-500 hover:duration-300">
        {text}
        <ChevronRightIcon size={14} />
      </span>
    </div>
