---
title: "Checkbox"
description: "Displays a simple or animated checkbox, respecting user motion preferences."
slug: "checkbox"
---

<Tabs defaultValue="preview">

<div className="flex items-center justify-between border-b dark:border-neutral-800/60 border-neutral-300/50 pr-3">

<TabsList>
	<TabsTrigger value="preview">Preview</TabsTrigger>
	<TabsTrigger value="code">Code</TabsTrigger>
</TabsList>

<CopyCode 
	code="default"
	example={true}
	mode="text"
/>

</div>

<TabsContent value="preview">

<ComponentView className="h-[350px] flex items-center justify-center">

<CheckboxExample />

</ComponentView>

</TabsContent>

<TabsContent value="code">

<CodeBlock className="h-[350px]" contentClassName="h-full">

```tsx
import { Checkbox } from "@/components/ui/checkbox";

export function CheckboxExample() {
  return (
    <form className="flex items-start gap-3">
      <Checkbox />
      <div className="grid gap-1.5">
        <label
          htmlFor="terms"
          className="text-sm font-medium leading-none text-black peer-disabled:cursor-not-allowed peer-disabled:opacity-70 dark:text-white"
        >
          Accept terms and conditions
        </label>
        <span className="text-sm text-neutral-500 dark:text-neutral-400">
          You agree to our Terms of Service and Privacy Policy.
        </span>
      </div>
    </form>
  );
}
```

</CodeBlock>

</TabsContent>

</Tabs>

<div className="space-y-2 pt-4">

<div className="pb-3 border-b dark:border-neutral-800/60 border-neutral-300/50">

## Usage

</div>

<CodeBlock className="mt-6" contentClassName="h-fit">

```tsx
import { Checkbox } from "@/components/ui/checkbox"
```
</CodeBlock>

<CodeBlock className="mt-4" contentClassName="h-fit">

```tsx
<Checkbox />
```
</CodeBlock>

</div>

<div className="space-y-2 pt-4">

## Installation

<Tabs defaultValue="cli">

<div className="flex items-center justify-between border-b dark:border-neutral-800/60 border-neutral-300/50 pr-3">

<TabsList>
	<TabsTrigger value="cli">CLI</TabsTrigger>
	<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

</div>

<TabsContent value="cli">

<CommandBlock 
	npmCommand="npx @luxeui/ui add checkbox" 
	pnpmCommand="pnpm dlx @luxeui/ui add checkbox" 
	yarnCommand="npx @luxeui/ui add checkbox" 
	bunCommand="bunx --bun @luxeui/ui add checkbox" 
/>

</TabsContent>

<TabsContent value="manual">

<div>

<div className="relative">

<div className="absolute flex h-9 w-9 select-none items-center justify-center rounded-full border-[3px] border-background bg-neutral-300 dark:bg-neutral-800">
	<span className="font-semibold text-primary">1</span>
</div>

<div className="ml-[1.1rem] border-l border-neutral-200 dark:border-neutral-900">
	
<div className="space-y-4 pb-10 pl-8 pt-1">

<h2 className="font-medium text-primary">

Install the package

</h2>

<CommandBlock 
	npmCommand="npm install motion @radix-ui/react-checkbox" 
	pnpmCommand="pnpm add motion @radix-ui/react-checkbox" 
	yarnCommand="yarn add motion @radix-ui/react-checkbox" 
	bunCommand="bun add motion @radix-ui/react-checkbox" 
/>

</div>
</div>
</div>

<div className="relative">

<div className="absolute flex h-9 w-9 select-none items-center justify-center rounded-full border-[3px] border-background bg-neutral-300 dark:bg-neutral-800">
	<span className="font-semibold text-primary">2</span>
</div>

<div className="ml-[1.1rem] border-l border-neutral-200 dark:border-neutral-900">
	
<div className="space-y-4 pb-10 pl-8 pt-1">

<h2 className="font-medium text-primary">

Copy and paste the following code into your project

</h2>

<CodeBlock fileName="checkbox.tsx">

```tsx
"use client"; // @NOTE: Add in case you are using Next.js

import * as RadixCheckbox from "@radix-ui/react-checkbox";

import { AnimatePresence, motion } from "motion/react";

import { cn } from "@/utils/cn";

type CheckboxProps = React.CustomComponentPropsWithRef<
  typeof RadixCheckbox.Root
>;

export function Checkbox(props: CheckboxProps) {
  const { checked } = props;

  return (
    <RadixCheckbox.Root
      {...props}
      className={cn(
        "relative inset-ring-1 inset-ring-border inline-block size-5 appearance-none rounded bg-neutral-100 dark:bg-neutral-900",
      )}
    >
      <AnimatePresence mode="popLayout">
        <RadixCheckbox.Indicator
          className={cn(
            "absolute inset-0 flex items-center justify-center rounded-[inherit] bg-primary",
          )}
          asChild
        >
          <motion.div
            initial={{
              opacity: 0,
              scale: 0.5,
            }}
            animate={{
              opacity: 1,
              scale: 1,
            }}
            transition={{
              duration: 0.3,
              ease: [0.175, 0.885, 0.32, 1.1],
            }}
          >
            <CheckIcon checkedState={checked} />
          </motion.div>
        </RadixCheckbox.Indicator>
      </AnimatePresence>
    </RadixCheckbox.Root>
  );
}

type CheckIconProps = {
  checkedState: CheckboxProps["checked"];
};

function CheckIcon({ checkedState }: CheckIconProps) {
  const CHECK_PATH = "M5 13 L10 18 L20 6";
  const INDETERMINATE_PATH = "M6 12 H18";

  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      strokeLinecap="round"
      strokeLinejoin="round"
      className={cn(
        "shrink-0 scale-65 stroke-4 stroke-white dark:stroke-black",
      )}
    >
      <title>Check</title>

      <motion.path
        d={checkedState === "indeterminate" ? INDETERMINATE_PATH : CHECK_PATH}
        initial={{
          pathLength: 0,
        }}
        animate={{
          pathLength: 1,
        }}
        transition={{
          duration: 0.3,
          ease: [0.645, 0.045, 0.355, 1],
        }}
      />
    </svg>
  );
}
```

</CodeBlock>

</div>
</div>
</div>

<div className="relative">

<div className="absolute flex h-9 w-9 select-none items-center justify-center rounded-full border-[3px] border-background bg-neutral-300 dark:bg-neutral-800">
	
<span className="font-semibold text-primary">3</span>

</div>

<div className="ml-[1.1rem] border-l border-neutral-200 dark:border-neutral-900">

<div className="space-y-4 pl-8 pt-1">

<h2 className="font-medium text-primary">

Update the import paths to match your project setup

</h2>

</div>
</div>
</div>
</div>

</TabsContent>

</Tabs>

</div>