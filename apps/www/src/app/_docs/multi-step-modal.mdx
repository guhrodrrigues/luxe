...
<MultiStepModal>
  <MultiStepModalTrigger>Multi Step Modal Trigger</MultiStepModalTrigger>
  <MultiStepModalContent steps={steps} />
</MultiStepModal>
```
...
<CommandBlock 
	npmCommand="npx @luxeui/ui add multi-step-modal" 
	pnpmCommand="pnpm dlx @luxeui/ui add multi-step-modal" 
	yarnCommand="npx @luxeui/ui add multi-step-modal" 
	bunCommand="bunx --bun @luxeui/ui add multi-step-modal" 
/>
  ...
  return (
    <MotionConfig transition={{ type: 'spring', duration: 0.6, bounce: 0 }}>
      <RadixDialog.Portal>
        <RadixDialog.Overlay
          className={cn(
            'fixed inset-0 bg-black/40 ease-out',
        ...
        <RadixDialog.Content
          ...
          className={cn(
            ...
            '-translate-x-1/2 fixed top-1/3 left-1/2 motion-safe:ease-out',
